...
class AppPrefs(private val sharedPreferences: SharedPreferences) {
    ...
    inner class Internal : ManagedPreferenceInternal(sharedPreferences) {
        ...
        val pinyinModeRime = string("input_method_pinyin_mode_rime", CustomConstant.SCHEMA_ZH_QWERTY) //拼音输入模式记录，记录引擎
        val inputDefaultMode = int("input_default_method_mode", inputMode)   //默认输入法类型
        val inputMethodPinyinMode = int("input_method_pinyin_mode", inputMode)  // 保存中文输入法类型
        ...
        val keyboardModeFloat = bool("keyboard_mode_float", true)     // 悬浮模式
        val keyboardModeFloatLandscape = bool("keyboard_mode_float_landscape", true)// 悬浮模式:横屏
    ...
    inner class KeyboardSetting : ManagedPreferenceCategory(R.string.setting_ime_keyboard, sharedPreferences) {
        ...
        val abcNumberLine = switch(R.string.engish_full_keyboard, "keyboard_abc_number_line_enable", true)
